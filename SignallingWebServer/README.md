...
22. Before using a Stage instance as a Prod AMI source, run `BuildScripts\prepare-for-ami-bake.ps1` after the final runtime artifact update. The helper verifies the artifact launch root, clears transient update/provisioning/runtime/session-artifact state while preserving active runtime-artifact identity, and does not mutate the launch root with git. `BuildScripts\prepare-scaleworld-s4-for-ami-bake.bat` is the local-on-instance runner for the Stage source named `ScaleWorld_s4`; `BuildScripts\prepare-scaleworld-s4-for-ami-bake-via-ssm.bat` runs the same operation from an operator workstation through SSM.
