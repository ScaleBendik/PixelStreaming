...
23. Every Unreal launch rotates through durable WebRTC UDP port banks by default. State is stored at `C:\PixelStreaming\state\webrtc-port-bank.json`; defaults are ports `49152-65535`, 256 ports per bank, the lowest/default bank permanently reserved, and an 11-minute no-reuse cooldown for the other 63 banks. `SCALEWORLD_WEBRTC_PORT_BANK_ROTATION_ENABLED=false` is the rollback switch. Range, bank size, cooldown, state path, and lock timeout are configurable with the `SCALEWORLD_WEBRTC_PORT_*` variables documented in `Docs/pixelstreaming-runtime-artifact-contract.md`. Do not change bank geometry while the stack is running or reuse/remove the state file during ordinary runtime-artifact activation.
