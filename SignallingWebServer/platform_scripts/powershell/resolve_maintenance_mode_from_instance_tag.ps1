...
try {
    $aws = Get-AwsCliPath
    if (-not $aws) {
        Write-Error "AWS CLI was not found while resolving ScaleWorldMaintenanceMode."
        exit 2
    }
    ...
    if ([string]::IsNullOrWhiteSpace($region)) {
        Write-Error "Failed to resolve the EC2 instance region while resolving ScaleWorldMaintenanceMode."
        exit 2
    }

    $tagValue = & $aws ec2 describe-tags `
        --region $region `
        --filters "Name=resource-id,Values=$instanceId" "Name=key,Values=ScaleWorldMaintenanceMode,ScaleWorldMaintenancemode" `
        --query 'Tags[0].Value' `
        --output text

    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to query the ScaleWorldMaintenanceMode instance tag in region '$region'."
        exit 2
    }
    ...
    if ($normalized -in @('provisioning', 'update', 'repo_sweep')) {
        Write-Output $normalized
    }
    exit 0
} catch {
    Write-Error "Failed to resolve the ScaleWorldMaintenanceMode instance tag. $($_.Exception.Message)"
    exit 2
}
